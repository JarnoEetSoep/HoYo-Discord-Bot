...
pub async fn run(database: &sqlx::SqlitePool, command: &ApplicationCommandInteraction, ctx: Arc<Context>) {
    ...
    if let CommandDataOptionValue::String(c) = command.data.options.get(0).unwrap().resolved.as_ref().unwrap() {
        code = c.clone();
    }
    ...
    for user in users {
        ...
        let output = tokio::task::spawn_blocking(move || {
            if let Err(error) = hoyo_client.claim_code(&code) {
                format!("Error claiming code `{}` on {}: `{}`", code, genshin_uid, error)
            } else {
...
pub fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
    command.name("claimcode").description("Claim code")
