...
use serenity::model::prelude::component::{InputTextStyle, ActionRowComponent::InputText};
...
pub async fn run(database: &sqlx::SqlitePool, command: &ApplicationCommandInteraction, ctx: Arc<Context>) {
    command.create_interaction_response(&ctx, |res| {
            ...
            .interaction_response_data(|msg| {
                msg.embed(|e| {
                    e.title("Link account")
                        ...
                        .colour((122, 71, 72))
                        .footer(|f| f.icon_url(command.user.avatar_url().unwrap())
                            .text(format!("Requested by {}#{}", command.user.name, command.user.discriminator)))
                        .timestamp(Timestamp::now())
                })
            })
    }).await.unwrap();

    let msg = command.user.direct_message(&ctx.http, |msg| {
        msg.components(|comp| {
    ...
    let mut interaction_stream = msg.await_component_interactions(&*ctx).timeout(Duration::from_secs(120)).build();
...
pub fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
    command.name("link").description("Link your HoYoLab account")
}

pub async fn link(database: &sqlx::SqlitePool, interaction: Arc<MessageComponentInteraction>, ctx: Arc<Context>) {
    ...
    let msg = interaction.user.direct_message(&ctx.http, |msg| {
        msg.components(|comp| {
            comp.create_action_row(|row| {
                row.create_button(|btn| {
                    btn.custom_id("cancel")
        ...
        if action == "proceed" {
            interaction.create_interaction_response(&ctx, |res| {
                res.kind(InteractionResponseType::Modal)
                    .interaction_response_data(|msg| {
                        msg.title("Link form").custom_id("link_form")
                            .components(|comp| {
                                comp.create_action_row(|row| {
                                    row.create_input_text(|input| {
                                        input.custom_id("genshin_uid")
                                            .label("Genshin UID:")
            ...
            let mut form_submit = modal_interaction_collector::ModalInteractionCollectorBuilder::new(&*ctx).build();

            while let Some(submission) = form_submit.next().await {
                if let InputText(genshin_uid) = submission.data.components.get(0).unwrap().components.get(0).unwrap() {
                    if let InputText(hoyo_cookie) = submission.data.components.get(1).unwrap().components.get(0).unwrap() {
                        ...
                        if let Err(error) = &destructured_cookie {
                            interaction.create_interaction_response(&ctx, |res| {
                                res.kind(InteractionResponseType::ChannelMessageWithSource)
                                    .interaction_response_data(|msg| {
                                        msg.content(format!("Could not link account:\n{}", error))
                                    })
                            }).await.unwrap();
                        ...
                        let (cookie, ltuid, ltoken, cookie_token, account_id, lang) = destructured_cookie.unwrap();
                        ...
                        let msg = submission.user.direct_message(&ctx.http, |msg| {
                            msg.components(|comp| {
                                comp.create_action_row(|row| {
                                    row.create_button(|btn| {
                                        btn.custom_id("cancel")
                        ...
                        while let Some(interaction) = interaction_stream.next().await {
                            ...
                            if action == "proceed" {
                                ...
                                if count_config == 0 {
                                    sqlx::query(format!("INSERT INTO config (genshin_uid) VALUES (\"{}\");", genshin_uid).as_str())
                                ...
                                let cookie_id = sqlx::query(format!("SELECT cookie_id FROM hoyo_cookie WHERE ltuid = \"{}\";", ltuid).as_str())
                                    .fetch_one(database).await.unwrap().get::<u32, _>(0);
