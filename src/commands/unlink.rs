...
pub fn register(command: &mut CreateApplicationCommand) -> &mut CreateApplicationCommand {
    command.name("unlink").description("Unlink your HoYoLab account")
}

pub async fn unlink(database: &sqlx::SqlitePool, interaction: Arc<MessageComponentInteraction>, ctx: Arc<Context>) {
    let discord_id = interaction.user.id.0.to_string();

    let users = sqlx::query(format!("SELECT genshin_uid FROM users WHERE discord_id = {};", discord_id).as_str())
        .fetch_all(database).await.unwrap();
    
    let users = users.into_iter().map::<String, _>(|user| user.get(0)).collect::<Vec<String>>();
    ...
    let msg = interaction.user.direct_message(&ctx, |msg| {
        msg.components(|comp| {
            comp.create_action_row(|row| {
    ...
    let interaction = match msg.await_component_interaction(&*ctx).timeout(Duration::from_secs(120)).await {
        Some(val) => {
            msg.delete(&ctx).await.unwrap();
    ...
    interaction.create_interaction_response(&ctx, |res| {
        res.kind(InteractionResponseType::ChannelMessageWithSource)
            .interaction_response_data(|msg| {
                msg.embed(|e| {
                    e.description(format!("Your account with uid: `{}` will be unlinked. Proceed?", genshin_uid))
                        .colour((122, 71, 72))
                })
            })
    }).await.unwrap();
    ...
    while let Some(interaction) = interaction_stream.next().await {
        ...
        if action == "proceed" {
            ...
            let query = sqlx::query(format!("DELETE FROM users WHERE (discord_id, genshin_uid) = (\"{}\", \"{}\");", discord_id, genshin_uid).as_str())
            ...
            if count_config == 1 {
                let query = sqlx::query(format!("DELETE FROM config WHERE genshin_uid = \"{}\";", genshin_uid).as_str())
                    .execute(database).await;
            ...
            if count_hoyo_cookie == 1 {
                ...
                let query = sqlx::query(format!("DELETE FROM hoyo_cookie WHERE cookie_id = \"{}\";", cookie_id).as_str())
                    .execute(database).await;
